...
| `/mesas` | `Mesas` | `src/pages/Mesas/Mesas.tsx` | Lista mesas (`GET /mesas`) em grade, com polling a cada 10 segundos (`setInterval`) para manter o status atualizado. Clicar numa mesa livre abre modal MUI para informar nome do cliente e criar pedido (`POST /pedidos`); clicar numa mesa com `pedido_atual_id` navega direto para `/pedido-items/:id`. |
